...
class StaffProvider extends ChangeNotifier {
  ...
  Future<void> scanCustomerQR(String qrCode) async {
    ...
    try {
      ...
      if (_scannedCustomer != null) {
        _customerBalance =
            await StaffService.getCustomerBalance(_scannedCustomer!.id);
      }
    } catch (e) {
      _error = e.toString();
    } finally {
  ...
  Future<bool> redeemCoupon(int staffId) async {
    ...
    try {
      final success =
          await StaffService.redeemCoupon(_scannedCoupon!.code, staffId);
      if (success) {
        // Refresh coupon status
        _scannedCoupon =
            await StaffService.getCouponByCode(_scannedCoupon!.code);
      }
      return success;
    } catch (e) {
      _error = e.toString();
