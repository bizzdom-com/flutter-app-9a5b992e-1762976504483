...
class AdminService {
  ...
  static Future<List<UserModel>> getAllStaff() async {
    try {
      final response = await _client.from('users').select().inFilter(
          'role', ['staff', 'manager']).order('created_at', ascending: false);
  ...
  static Future<bool> deleteStaffMember(int userId) async {
    try {
      ...
      await _client.from('staff').delete().eq('user_id', userId);

      // Demote user to customer instead of deleting
      await _client.from('users').update({'role': 'customer'}).eq('id', userId);

      // Create customer balance if it doesn't exist
      try {
        await _client.from('customer_balances').insert({
          'user_id': userId,
          'points_balance': 0,
        });
      } catch (e) {
  ...
  static Future<Map<String, dynamic>> getDashboardStats() async {
    try {
      ...
      // Get active coupons
      final activeCouponsResponse =
          await _client.from('coupons').select('id').eq('status', 'active');
  ...
  static Future<List<PointsLedgerModel>> getRecentTransactions(
      {int limit = 20}) async {
    try {
      final response = await _client
          .from('points_ledger')
