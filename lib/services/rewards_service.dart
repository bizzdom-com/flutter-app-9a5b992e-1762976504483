  ...
  static Future<CouponModel?> redeemReward({
    ...
    try {
      ...
      if (balance < costPoints) {
        throw Exception(
            'Insufficient points. You have $balance points but need $costPoints.');
      }
      ...
      await _client.from('customer_balances').update(
          {'points_balance': balance - costPoints}).eq('user_id', userId);
